...
def find_persons(frame):
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        top = int(top / ratio)
        right = int(right / ratio)
        bottom = int(bottom / ratio) + 60
        left = int(left / ratio)
        cv2.rectangle(frame, (left, top), (right, bottom), (205, 205, 0), int(1 / ratio))
        cv2.rectangle(frame, (left, bottom - 30), (right, bottom), (205, 205, 0), cv2.FILLED)
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(frame, name, (left + 5, bottom - 5), font, 0.9, (255, 255, 255), 1)
