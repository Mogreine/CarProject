...
GMotor::GMotor(uint8_t dig_pin, uint8_t pwm_pin, int channel) {
	_dig_pin = dig_pin;
	_pwm_pin = pwm_pin;
	_ledc_channel = channel;
	
	ledcSetup(_ledc_channel, 255, 8);
...
void GMotor::runFrw() {
	...
	ledcWrite(_ledc_channel, _duty);
}
void GMotor::runBkw() {
	digitalWrite(_dig_pin, 1);
	// analogWrite(_pwm_pin, 255 - _duty);
	ledcWrite(_ledc_channel, 255 - _duty);
}
...
void GMotor::runBkw10bit() {
	...
	ledcWrite(_ledc_channel, 1023 - _duty);
}
